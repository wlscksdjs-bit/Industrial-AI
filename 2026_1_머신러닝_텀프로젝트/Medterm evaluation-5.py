# ==========================================
# 충북대학교 산업인공지능학과 중간 프로젝트 (고도화 버전 3.0)
# 주제: OAC vs 순환기 케미컬 필터 교체 예측 (Warning Window 도입)
# 평가: Train 6 : Validation 2 : Test 2 (Accuracy, Precision, Recall, F1)
# ==========================================
...
# ==========================================
# ★ [핵심 솔루션] 타겟 엔지니어링: Warning Window (예지보전 경고 구간)
# ==========================================
combined_df['Filter_Replace_Warning'] = 0
if len(filter_actions) > 0:
    pm_dates = filter_actions['TIMESTAMP'].dt.floor('D').unique()
    for pm_date in pm_dates:
        # 필터 교체 당일 및 교체 전 3일을 모두 '경고 구간(1)'으로 설정 (정확도 대폭 상승 원리)
        mask = (combined_df.index <= pm_date) & (combined_df.index >= pm_date - pd.Timedelta(days=3))
...
for col in ion_cols:
    ...
    combined_df[f'{col}_Gen_MA7'] = combined_df[f'{col}_Internal_Gen'].rolling(window=7).mean()  # 7일 장기 누적량 추가
    combined_df[f'{col}_OAC_MA3'] = combined_df[f'{col}_OAC'].rolling(window=3).mean()

combined_df.dropna(inplace=True)

# [Step 3] 데이터 분할 (6:2:2)
drop_cols = ['Filter_Replace_Warning'] + [f'{col}_BAY' for col in ion_cols]
...
# ==========================================
# ★ [결과 출력] 종합 평가지표 (Accuracy, Precision, Recall, F1)
# ==========================================
...
print("\n=== 케미컬 필터 예지보전(Warning Window) 예측 성능 ===")
print_metrics("Validation (검토)", y_val, l1_model.predict(X_val_scaled))
print_metrics("Test (평가)", y_test, l1_model.predict(X_test_scaled))
...
plt.legend(handles=[
    mpatches.Patch(color='tomato', label='내부 설비 누출 가스 (Internal)'),
    mpatches.Patch(color='mediumseagreen', label='외부 유입 가스 (OAC)')
])
plt.title("케미컬 필터 교체(Warning) 유발 '킬러 가스' 영향력", fontsize=14)
plt.grid(True, alpha=0.3)
