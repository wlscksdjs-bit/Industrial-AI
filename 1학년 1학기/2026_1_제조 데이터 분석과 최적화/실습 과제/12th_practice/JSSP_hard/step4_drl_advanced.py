...
class JSSPMaskableEnv(gym.Env):
    ...
    def __init__(self, job_data, machines):
        ...
        # 이론적 하한값 계산 (가장 긴 작업의 총 가공시간)
        self.lower_bound = max([sum(t for _, t in self.job_data[j]) for j in self.job_names])
    ...
    def step(self, action):
        ...
        if done:
            ...
            # 최적해 하한값(lower_bound) 기반으로 유동적 보상 부여
            reward += (self.lower_bound * 2.0 - makespan) * 2
...
def training_curve_experiment(job_data, machines, verbose=True):
    ...
    timestep_list = [5000, 10000, 30000, 60000, 100000, 150000]
    results = []
    ...
    for ts in timestep_list:
        ...
        if verbose:
            bar_len = max(0, int((100 - ms) / 100 * 30)) if ms < 999 else 0
...
if __name__ == '__main__':
    ...
    # ── Maskable PPO 실행 ──
    print("  ─── Maskable PPO (100,000 스텝) ───")
    df, ms, et = run_maskable_ppo(JOB_DATA, MACHINES, total_timesteps=100000)
